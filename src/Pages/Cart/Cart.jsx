...
function Cart() {
  ...
  async function fetchCartDetails() {
    const data = await dispatch(getCartDetails());
    console.log(data);
    setCartDetails(data?.payload?.data.data);
  }
                        ...
                        <div className="flex-1 w-full min-w-0 md:order-2 md:max-w-md">
                          <p className="text-base font-medium text-gray-900 hover:underline">
                            <Link to={`/product/${item?._id}`}>
                              {`${item?.product?.productName}, ${item?.product?.descripition}, Category: ${item?.product?.category}`}
                            </Link>
                          </p>
